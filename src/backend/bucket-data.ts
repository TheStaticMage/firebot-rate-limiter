...
import { Bucket, BucketDataEntry, CheckRateLimitRequest, CheckRateLimitResponse, RejectReason } from '../shared/types';
...
class BucketData {
    ...
    check(request: CheckRateLimitRequest): CheckRateLimitResponse {
        const bucket = this.getBucket(request.bucketId);
        if (!bucket) {
    ...
    private getBucket(bucketId: string): Bucket | undefined {
        return this.bucketService.getBucket(bucketId);
    }
