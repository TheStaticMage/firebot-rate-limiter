...
import { modifyEffectModel, NonCriticalError, processTrigger } from "./modify";
...
describe("processTrigger", () => {
    ...
    it("handles negative and zero values for add/set operations", () => {
        ...
        expect(result.rateLimitModifyBucketDataSuccess).toBe("true");
        expect(baseBucketData.setKey).toHaveBeenCalledWith(effect.bucketId, "user:alice", expect.objectContaining({ tokenCount: 0 }));
    });
    ...
    it("modifies all user keys with add operation", () => {
        ...
        expect(result.rateLimitModifyBucketDataSuccess).toBe("true");
        expect(baseBucketData.setKey).toHaveBeenCalledTimes(2);
        expect(baseBucketData.setKey).toHaveBeenCalledWith(effect.bucketId, "user:alice", expect.objectContaining({ tokenCount: 12 }));
        expect(baseBucketData.setKey).toHaveBeenCalledWith(effect.bucketId, "user:bob", expect.objectContaining({ tokenCount: 12 }));
    });
    ...
    it("modifies all keys with set operation", () => {
        ...
        expect(result.rateLimitModifyBucketDataSuccess).toBe("true");
        expect(baseBucketData.setKey).toHaveBeenCalledTimes(3);
        expect(baseBucketData.setKey).toHaveBeenCalledWith(effect.bucketId, "user:alice", expect.objectContaining({ tokenCount: 99 }));
        expect(baseBucketData.setKey).toHaveBeenCalledWith(effect.bucketId, "custom:foo", expect.objectContaining({ tokenCount: 99 }));
        expect(baseBucketData.setKey).toHaveBeenCalledWith(effect.bucketId, "global", expect.objectContaining({ tokenCount: 99 }));
    });
